...
#ORIGINAL SUBMITTED METHOD BELOW (2 SPECS NOT PASSING)
# class String
...
#FIXED METHOD BELOW (PASSES ALL SPECS)
class String
  def shuffled_sentence_detector(other)
    self_words = self.split(/\s+/)
    other_words = other.split(/\s+/)

    return true if other_words.sort == self_words.sort
    false
  end
end
